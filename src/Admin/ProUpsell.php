...
/**
 * PRO upgrade promotion, shown ONLY on the Returns settings screen: a
...
{
    private const META   = 'returns_pro_banner_dismissed';
    private const ACTION = 'returns_dismiss_pro';
    ...
    {
        if ($this->data === null) {
            $file = RETURNS_DIR . 'config/pro-upsell.php';
    ...
    {
        /**
         * Filters whether the Returns PRO promo is shown on the settings screen.
         *
         * @param bool $show Default true.
         */
        return (bool) apply_filters('returns/show_pro_cta', true) && $this->features() !== [];
    }

    private function url(): string
    {
        $default = (string) ($this->data()['url'] ?? 'https://plogins.com/plogins-returns-pro/pricing/');
        /**
         * Filters the URL the "Upgrade to PRO" buttons point at.
         *
         * @param string $url Default the Returns PRO pricing page.
         */
        return (string) apply_filters('returns/pro_url', $default);
    }
    ...
    private function priceLabel(): string
    {
        ...
        if ($this->isPolish() && ! empty($d['price_pln'])) {
            ...
            return sprintf(__('od %d zł/rok', 'plogins-returns'), (int) $d['price_pln']);
        }
    ...
    public function handleDismiss(): void
    {
        ...
        wp_safe_redirect(wp_get_referer() ?: admin_url('admin.php?page=returns-settings'));
        exit;
    }
    ...
    public function banner(): void
    {
        ...
        ?>
        <div class="returns-pro-banner" role="note">
            <span class="returns-pro-banner__tag">PRO</span>
            ...
            <a class="button button-primary returns-pro-banner__cta" href="<?php echo esc_url($this->url()); ?>" target="_blank" rel="noopener noreferrer">
                <?php esc_html_e('Upgrade to PRO', 'plogins-returns'); ?>
            </a>
        ...
        <aside class="returns-card returns-pro-aside" aria-labelledby="returns-pro-aside-h">
            ...
            <ul class="returns-pro-aside__list">
                <?php foreach ($features as $f) : ?>
                    <li>
                        <span class="returns-pro-aside__lock" aria-hidden="true"></span>
                        <span><?php echo esc_html($f['title']); ?></span>
                    </li>
                <?php endforeach; ?>
            </ul>
            <a class="button button-primary button-hero returns-pro-aside__cta" href="<?php echo esc_url($this->url()); ?>" target="_blank" rel="noopener noreferrer">
                <?php esc_html_e('Upgrade to PRO', 'plogins-returns'); ?>
            </a>
            <?php if ($price !== '') : ?>
                <p class="returns-pro-aside__price"><?php echo esc_html($price); ?> · <?php esc_html_e('one licence, every PRO feature', 'plogins-returns'); ?></p>
            <?php endif; ?>
        </aside>
        <?php
    }
    ...
    public function cards(): void
    {
        ...
        <section class="returns-pro-cards" aria-labelledby="returns-pro-cards-h">
            ...
            <div class="returns-pro-cards__grid">
                <?php foreach ($features as $f) : ?>
                    <article class="returns-pro-card">
                        <span class="returns-pro-card__badge">PRO</span>
                        <span class="returns-pro-card__lock" aria-hidden="true"></span>
                        <h3 class="returns-pro-card__title"><?php echo esc_html($f['title']); ?></h3>
                        <?php if ($f['desc'] !== '') : ?>
                            <p class="returns-pro-card__desc"><?php echo esc_html($f['desc']); ?></p>
                        <?php endif; ?>
                    </article>
                <?php endforeach; ?>
            </div>
